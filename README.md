...
每款游戏的数据集中保存在 `data/games/<游戏 ID>/`，包含 `library`、`downloads`、`previews`、`backgrounds` 和游戏记录。全局设置与应用会话保存在 `data`；备份或移动程序时请保留整个目录。本项目不自动迁移旧版分散的数据目录。

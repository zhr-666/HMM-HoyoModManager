...
test('mod downloads require the selected game loader and its Mods directory',async t=>{const root=await fs.mkdtemp(path.join(os.tmpdir(),'hoyo-loader-'));t.after(()=>fs.rm(root,{recursive:true,force:true}));await assert.rejects(requireMods({},'zzz'),/ZZMI/);await assert.rejects(requireMods({modsPath:root},'zzz'),/ZZMI.*d3dx.ini/);const modsPath=path.join(root,'Mods');await fs.mkdir(modsPath);await fs.writeFile(path.join(root,'d3dx.ini'),'[Include]');await requireMods({modsPath},'zzz');});

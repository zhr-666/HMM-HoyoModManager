...
function showHashReplace(){
  ...
  $('#preview-hash').onclick=async()=>{
    try{const result=await call('previewHash',{oldHash:$('#old-hash').value,newHash:$('#new-hash').value});
      ...
      $('#apply-hash')?.addEventListener('click',async()=>{closeModal();try{const batch=await call('applyHash',{token:result.token},{reload:true});notify(`已替换 ${batch.count} 处，可在“替换记录”回滚。`)}catch{}});
    }catch{}
  };
}
...
$('#home-background').onerror=()=>{if($('#home-background').getAttribute('src')!=='home-background.jpg'){$('#home-background').src='home-background.jpg';notify('自定义背景无法读取，已使用默认背景。',true);}};

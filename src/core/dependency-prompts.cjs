...
class DependencyPrompts{
 constructor(send){this.send=send;this.queue=[];}
 ...
 answer(token,accepted){const row=this.current(token);this.queue.shift();row.resolve(accepted===true);if(this.queue.length)this.send(this.queue[0].detail);}
 link(token,index){
  const rows=this.current(token).detail.missing;

...
async function kuroBackgroundEntry(json){
  ...
  const details=await json(`${BASE_URL}${hash}/zh-Hans.json`,headers),image=details?.firstFrameImage,video=details?.backgroundFile;
  ...
  if(video&&(details.backgroundFileType!==2||!/^https:\/\/hw-pcdownload-qcloud\.aki-game\.net\/launcher\/clientUpload\/[A-Za-z0-9_-]+\.mp4$/.test(video)))throw Error('鸣潮官方背景视频不可用。');
  ...
  const movie=video&&config?.default?.cdnList?.some(item=>item?.url===aws)?video.replace('https://hw-pcdownload-qcloud.aki-game.net/',aws):video;
  return {backgrounds:[{background:{url:poster},...(movie?{video:{url:movie}}:{})}]};
}

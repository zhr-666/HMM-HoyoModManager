...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-page="library"]').click();assert.equal(await page.locator('#open-mods-button').isVisible(),true);await page.locator('#open-managed-mods-button').click();await page.locator('#open-mods-button').click();assert.deepEqual(await app.evaluate(()=>testOpened.slice(-2)),[path.join(mods,'HoYoModManaged'),mods]);

...
async function main(){
 ...
 try{
  ...
  assert.equal(await evalJs(`(()=>{const tip=document.querySelector('#onboarding-tip'),box=tip.getBoundingClientRect(),button=document.querySelector('.rail-games').getBoundingClientRect(),arrow=parseFloat(getComputedStyle(tip).getPropertyValue('--arrow-offset'));return Math.abs(box.top+arrow-(button.top+button.height/2))<=2})()`),true,'第一步尖角应指向全部游戏按钮');
  ...
  assert.equal(await evalJs(`(()=>{const tip=document.querySelector('#onboarding-tip'),box=tip.getBoundingClientRect(),button=document.querySelector('#game-choose-mods').getBoundingClientRect(),arrow=parseFloat(getComputedStyle(tip).getPropertyValue('--arrow-offset'));return Math.abs(box.left+arrow-(button.left+button.width/2))<=2})()`),true,'目录尖角应指向选择按钮');
  ...
  await evalJs(`document.querySelector('#modal button[value="cancel"]').dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0}));document.querySelector('#modal button[value="cancel"]').click()`);
  ...
  assert.equal(await evalJs(`(()=>{const tip=document.querySelector('#onboarding-tip'),box=tip.getBoundingClientRect(),target=document.querySelector('#game-list .game-tile.active').getBoundingClientRect(),arrow=parseFloat(getComputedStyle(tip).getPropertyValue('--arrow-offset'));return Math.abs(box.top+arrow-(target.top+target.height/2))<=2})()`),true,'尖角应指向游戏图标中心');
  await evalJs(`document.body.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0}));openGameSettings();document.querySelector('#modal button[value="cancel"]').dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0}));document.querySelector('#modal button[value="cancel"]').click()`);
  await sleep(200);
  assert.equal(await evalJs(`document.querySelector('#onboarding-tip').matches(':popover-open')`),false,'主动收起后关闭其他设置窗口不应重现提示');
  await evalJs(`setOnboardingStep(3)`);
  await wait(`document.querySelector('#onboarding-tip').matches(':popover-open')`,'重新显示双击提示');

...
(async()=>{
  ...
  try{
    ...
    // 1.1.3 起只有深色模式：设置里不再有主题选择，直接检查材质选项的可用性。
    assert.equal(await page.locator('#theme-select').count(),0,'只有深色模式，不该再有主题选择');
    ...
    await page.locator('#notification-toast').waitFor({state:'visible'});
    assert.match(await page.locator('#notification-toast').textContent(),/已加入下载列表/);
    assert.equal(await page.locator('#notification-toast').evaluate(el=>getComputedStyle(el).animationName),'toast-enter');
    assert.equal(await page.locator('#page-workshop').isVisible(),true);
    await page.evaluate(()=>window.scrollTo(0,650));
    const toastBox=await page.locator('#notification-toast').boundingBox();
    const viewport=await page.evaluate(()=>({w:window.innerWidth,h:window.innerHeight}));
    assert.ok(toastBox.y+toastBox.height>viewport.h/2,'即时通知从右下角弹出（滚动页面后仍停在窗口右下）');
    assert.ok(toastBox.x+toastBox.width>viewport.w/2,'即时通知靠右对齐');
    await page.screenshot({path:path.resolve(__dirname,'../test-results/notification-toast.png')});
    await page.locator('#notification-toast').waitFor({state:'hidden',timeout:5000});
    ...
    console.log('Renderer passed: hierarchical categories, parent browsing, breadcrumbs, scoped category search, refresh retention, stale responses, library switch and clear previews, configuration gate, dark-only appearance, workshop filters/page/scroll retention, background queue and retry, manual update summary.');

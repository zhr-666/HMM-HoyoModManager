...
(async()=>{
  ...
  try{
    ...
    const toastBox=await page.locator('#download-toast').boundingBox();
    const viewport=await page.evaluate(()=>({w:window.innerWidth,h:window.innerHeight}));
    assert.ok(toastBox.y+toastBox.height>viewport.h/2,'下载提示现在从右下角弹出（滚动页面后仍停在窗口右下）');
    assert.ok(toastBox.x+toastBox.width>viewport.w/2,'下载提示靠右对齐');

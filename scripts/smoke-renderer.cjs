...
(async()=>{
  ...
  try{
    ...
    await page.addInitScript(()=>{
      ...
      window.hoyo={
        call:async(action,p={})=>{
          ...
          if(action==='taxonomy')return [{id:20,name:'Audio',children:[{id:21,name:'Music',children:[{id:22,name:'Battle',children:[]}]}]},{id:17510,name:'Skins',children:[{id:1,name:'角色 A',children:[]},{id:2,name:'角色 B',children:[]}]}];
    ...
    await page.locator('.category').first().waitFor();assert.equal(await page.locator('#root-category-select').inputValue(),'17510');
    await page.locator('.category').nth(1).click();await page.locator('.category').nth(2).click();await page.waitForTimeout(600);
    ...
    await page.locator('[data-page="workshop"]').click();
    assert.equal(await page.locator('.mod-card').count(),20);
    assert.equal(await page.locator('.category.active .category-name').textContent(),'角色 B');
    ...
    await page.locator('[data-page="settings"]').click();await page.locator('#theme-select').selectOption('light');await page.waitForFunction(()=>document.documentElement.dataset.theme==='light');
    await page.locator('[data-page="workshop"]').click();assert.equal(await page.evaluate(()=>window.scrollY),scroll);assert.match(await page.locator('#page-info').textContent(),/第 2 页/);
    assert.equal(await page.evaluate(()=>calls.filter(x=>x.action==='browse').length),callsBefore+1);
    await page.locator('[data-page="library"]').click();assert.equal(await page.locator('.folder-card').filter({hasText:'Skins'}).count(),1);await page.locator('.folder-card').filter({hasText:'Skins'}).click();await page.locator('.folder-card').filter({hasText:'角色 A'}).click();assert.equal(await page.locator('.library-thumb .nsfw-image').count(),0);
    ...
    await page.locator('[data-page="downloads"]').click();assert.equal(await page.locator('[data-testid="download-row"]').count(),1);assert.match(await page.locator('.download-progress').textContent(),/50%/);
    ...
    assert.equal(await page.locator('#launch-button').isEnabled(),true);
    ...
    await page.evaluate(()=>window.configureMods());await page.locator('[data-page="settings"]').click();await page.locator('#refresh-button').click();
    await page.locator('[data-page="workshop"]').click();await page.locator('#root-category-select').selectOption('20');await page.waitForTimeout(100);
    assert.equal(await page.locator('.category').count(),3);assert.match(await page.locator('.category').last().getAttribute('title'),/Audio \/ Music \/ Battle/);
    await page.locator('.category').last().click();await page.waitForTimeout(100);await page.locator('.detail').first().click();assert.equal(await page.locator('#detail-character').count(),0);
    ...
    await page.locator('[data-page="library"]').click();await page.locator('#check-updates-library').click();
    ...
    assert.match(await page.locator('.update-window-note').textContent(),/72 小时/);assert.deepEqual(errors,[]);
    console.log('Renderer passed: stale responses, library switch and clear previews, configuration gate, theme, workshop filters/page/scroll retention, background queue and retry, manual update summary.');

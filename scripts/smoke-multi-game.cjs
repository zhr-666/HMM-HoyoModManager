...
async function main(){
 ...
 try{
  ...
  await waitFor(`!document.querySelector('#context-menu').hidden`,'游戏右键菜单');
  assert.equal(await evaluate(`document.querySelector('#modal').open`),false,'右键不应直接打开删除确认');
  assert.deepEqual(await evaluate(`[...document.querySelectorAll('#context-menu [role="menuitem"]:not([hidden])')].map(el=>el.textContent.trim())`),['删除游戏']);
  await fs.access(path.join(data,'games','wuwa','state.json'));
  ...
  await evaluate(`window.originalDetailCall=api.call;api.call=async(action,p)=>action==='detail'?{id:p.id,name:'图片回归',images:['https://images.gamebanana.com/one.png','https://images.gamebanana.com/two.png'],files:[{id:1,name:'one.zip',size:1024,downloadCount:1234},{id:2,name:'two.zip',size:2048,downloadCount:0}]}:window.originalDetailCall(action,p);openDetail({id:1,name:'图片回归'})`);
  await waitFor(`!!document.querySelector('#modal .file-option')`,'详情文件和图片');
  assert.deepEqual(await evaluate(`[...document.querySelectorAll('#modal .file-option small')].map(el=>el.textContent.includes('次下载')?el.textContent.match(/[0-9,]+ 次下载/)?.[0]:null)`),['1,234 次下载','0 次下载']);
  assert.equal(await evaluate(`getComputedStyle(document.querySelector('.detail-gallery-main')).cursor`),'pointer');
  await evaluate(`document.querySelector('.detail-gallery-main').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal').open`),true);
  assert.equal(await evaluate(`document.querySelector('#image-modal .image-count').textContent`),'1 / 2');
  await evaluate(`document.querySelector('#image-next').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal .image-count').textContent`),'2 / 2');
  await evaluate(`document.querySelector('#image-prev').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal .image-count').textContent`),'1 / 2');
  await evaluate(`document.querySelector('#image-modal .image-close').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal').open`),false);
  assert.equal(await evaluate(`document.querySelector('#modal').open`),true,'关闭放大图后详情窗口仍在');
  await evaluate(`document.querySelector('.detail-gallery').classList.add('nsfw-detail');document.querySelector('.detail-gallery-main').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal').open`),false,'NSFW 首次点击只解除模糊');
  await evaluate(`document.querySelector('.detail-gallery-main').click()`);
  assert.equal(await evaluate(`document.querySelector('#image-modal').open`),true,'解除模糊后可以放大');
  await evaluate(`document.querySelector('#image-modal .image-close').click()`);
  await evaluate(`document.querySelector('#modal .dialog-back').click()`);
  await evaluate(`api.call=window.originalDetailCall;delete window.originalDetailCall`);
  assert.deepEqual(await evaluate(`(()=>{const hint=document.querySelector('#import-button .hint-mark'),other=document.querySelector('#page-settings .hint-mark');return [getComputedStyle(hint).cursor,getComputedStyle(hint).color===getComputedStyle(other).color]})()`),['default',true]);

...
(async()=>{
  ...
  try {
    ...
    assert.match(await page.locator('#hash-panel').textContent(),/2 个模组、2 个文件、2 处匹配/);
    await page.locator('#apply-hash').click();
    await page.waitForFunction(async()=>((await window.hoyo.call('state')).hashBatches||[]).length===1);
    // 替换完成后自动切到「替换记录」，记录里能看到换了什么值。
    await page.waitForFunction(()=>document.querySelector('[data-hash-tab="records"]')?.getAttribute('aria-selected')==='true');
    assert.match(await page.locator('#hash-panel').textContent(),/aabbccdd → 11223344/);
    await page.locator('[data-hash-tab="rollback"]').click();
    await page.locator('.hash-rollback').first().click();
    await page.locator('.hash-rollback-confirm').click();
    await page.waitForFunction(async()=>(await window.hoyo.call('state')).hashBatches[0].status==='rolledBack');
    await page.waitForFunction(()=>/回溯记录/.test(document.querySelector('#hash-panel').textContent)&&document.querySelector('#modal').open);
    await page.locator('#modal-actions button').click();

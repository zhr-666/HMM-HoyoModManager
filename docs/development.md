...
- 一个任务使用一个主题明确的分支，新分支默认 `codex/<任务名>`；有未提交工作时先明确归属，再决定提交、独立工作区或留在当前任务完成，不能为创建分支擅自 stash 或重置。
...
- [ ] 按语义化版本意图更新 package.json：修复升补丁，兼容功能升次版本，不兼容变化明确说明；当前 0.x 阶段也要单列迁移影响。
